...
function SendNotification(phoneNumberOrSource, notificationData, callback)
    ...
    if type(notificationData) ~= "table" or not notificationData.app or type(phoneNumberOrSource) ~= "string" then
        if callback then
            callback(false)
        end
...
function NotifyPhones(fromTable, notificationData, selectPrefix, parameters)
    if table.contains(disabledNotifications, notificationData.app) then
