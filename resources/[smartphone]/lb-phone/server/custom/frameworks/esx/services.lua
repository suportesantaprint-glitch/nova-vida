...
RegisterNetEvent("vRP:setJob") -- Ajuste o nome se a sua base usar um evento customizado de job
AddEventHandler("vRP:setJob", function(source, job)
    -- O terceiro parâmetro (lastJob) pode ser enviado como nil se o vRP não fornecer
    OnJobUpdate(source, job, nil)
end)

-- Evento de quando o jogador spawna/carrega na Creative (vRP)
RegisterNetEvent("vRP:playerSpawned")
AddEventHandler("vRP:playerSpawned", function(user_id, source)
    local src = source or source
    -- Na Creative, você pode precisar buscar o job do jogador via export ou tabela
    -- Como solução segura de carregamento, chamamos a inicialização do celular:
    TriggerServerEvent("lb-phone:server:playerLoaded")
    
    -- Se o seu script tiver a função de atualizar emprego, você pode chamá-la aqui
    -- OnJobUpdate(src, "desempregado") 
end)

-- Evento de quando o jogador desloga do personagem (Creative)
RegisterNetEvent("vRP:playerDropped")
AddEventHandler("vRP:playerDropped", function(user_id, source)
    local src = source
    UnloadJob(src)
end)
