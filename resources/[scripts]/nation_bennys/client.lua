...
local vSERVER = Tunnel.getInterface("nation_bennys")

local function GetPlateFromVeh(veh)
  local p = GetVehicleNumberPlateText(veh)
  if p then
    p = p:gsub("%s+", "") -- remove espaços
  end
  return p
end
...
AddEventHandler('abrirpainel',function()
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        TriggerEvent("Mecânica","Você precisa estar em um veiculo para abrir o painel de tunagem.",5000,"error")
    end
end)
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- Bennys(myveh) -> Creative garages(customize) [ÚNICA FUNÇÃO - SEM DUPLICAR]
-----------------------------------------------------------------------------------------------------------------------------------------
-- -----------------------------------------------------------------------------------------------------------------------------------------
-- HELPERS (compat garagens)
-- -----------------------------------------------------------------------------------------------------------------------------------------
local function EnsureNeonTable(neon)
	-- aceita boolean (tem kit) ou table [0..3]/{"0"..}
	...
	if type(neon) == "table" then
		for i = 0, 3 do
			local v = neon[i]
			if v == nil then v = neon[tostring(i)] end
			out[i] = (v == true)
			out[tostring(i)] = (v == true)
		end
	end
	return out
end

-- Bennys(myveh) -> Creative garages(customize)
local function ToGarageCustomize(myveh)
	myveh = myveh or {}
	local customize = {}

	-- cores base (paleta)
	local primary, secondary = 0, 0
	if type(myveh.color) == "table" then
		primary = tonumber(myveh.color[1] or myveh.color["1"]) or 0
		secondary = tonumber(myveh.color[2] or myveh.color["2"]) or 0
	end

	-- extra colors
	local pear, wheelc = 0, 0
	if type(myveh.extracolor) == "table" then
		pear = tonumber(myveh.extracolor[1] or myveh.extracolor["1"]) or 0
		wheelc = tonumber(myveh.extracolor[2] or myveh.extracolor["2"]) or 0
	end

	-- custom RGB (se existir)
	customize.customPcolor = myveh.customPcolor
	customize.customScolor = myveh.customScolor
	customize.isCustomPrimary = myveh.isCustomPrimary == true
	customize.isCustomSecondary = myveh.isCustomSecondary == true

	-- neon
	customize.neon = EnsureNeonTable(myveh.neon)

	-- neon color -> lights (garages usa lights)
	local nr, ng, nb = 255, 255, 255
	if type(myveh.neoncolor) == "table" then
		nr = tonumber(myveh.neoncolor[1] or myveh.neoncolor["1"]) or 255
		ng = tonumber(myveh.neoncolor[2] or myveh.neoncolor["2"]) or 255
		nb = tonumber(myveh.neoncolor[3] or myveh.neoncolor["3"]) or 255
	end
	customize.lights = { nr, ng, nb }

	-- smoke
	local sr, sg, sb = 255, 255, 255
	if type(myveh.smokecolor) == "table" then
		sr = tonumber(myveh.smokecolor[1] or myveh.smokecolor["1"]) or 255
		sg = tonumber(myveh.smokecolor[2] or myveh.smokecolor["2"]) or 255
		sb = tonumber(myveh.smokecolor[3] or myveh.smokecolor["3"]) or 255
	end
	customize.smoke = { sr, sg, sb }
	customize.smokecolor = { sr, sg, sb }

	-- window tint (garages usa tint / window)
	local tint = 0
	if myveh.windowtint ~= false and myveh.windowtint ~= nil then
		tint = tonumber(myveh.windowtint) or 0
	end
	customize.tint = tint
	customize.window = tint

	-- xenon
	customize.xenonColor = tonumber(myveh.xenoncolor) or -1
	customize.xenon = customize.xenonColor

	-- placa
	customize.plateIndex = tonumber(myveh.plateindex) or 0
	customize.plate = customize.plateIndex

	-- tyres
	customize.tyresCanBurst = (myveh.bulletProofTyres == nil and true) or myveh.bulletProofTyres
	customize.tyres = customize.tyresCanBurst and 1 or 0

	customize.wheeltype = tonumber(myveh.wheeltype) or 0
	customize.model = myveh.model

	-- ✅ nomes esperados pela garage
	customize.colors = { primary, secondary }
	customize.color  = { primary, secondary }

		-- ✅ cores RGB (pintura "custom") — precisa persistir ao guardar/puxar
		local cP = myveh.customPcolor or { 0,0,0 }
		local cS = myveh.customScolor or { 0,0,0 }
		-- table.pack às vezes vira chaves "1","2","3"
		local cpr = tonumber(cP[1] or cP["1"]) or 0
		local cpg = tonumber(cP[2] or cP["2"]) or 0
		local cpb = tonumber(cP[3] or cP["3"]) or 0
		local csr = tonumber(cS[1] or cS["1"]) or 0
		local csg = tonumber(cS[2] or cS["2"]) or 0
		local csb = tonumber(cS[3] or cS["3"]) or 0
		customize.customPcolor = { cpr, cpg, cpb }
		customize.customScolor = { csr, csg, csb }
		customize.isCustomPrimary = myveh.isCustomPrimary and true or false
		customize.isCustomSecondary = myveh.isCustomSecondary and true or false
		-- aliases (caso outro script use)
		customize.customPrimary = customize.customPcolor
		customize.customSecondary = customize.customScolor

	customize.extracolors = { pear, wheelc }
	customize.extracolor  = { pear, wheelc }

	-- mods e var (0..48) no formato que a garage espera
	customize.mods = {}
	customize.var  = {}
	if type(myveh.mods) == "table" then
		for i = 0, 48 do
			local t = myveh.mods[i] or myveh.mods[tostring(i)]
			local val = -1
			local v = 0

			if type(t) == "table" then
				val = tonumber(t.mod) or -1

				if (i == 23 or i == 24) and t.variation ~= nil then
					if type(t.variation) == "boolean" then
						v = t.variation and 1 or 0
					else
						v = tonumber(t.variation) or 0
					end
				end
			...
			customize.mods[i] = val
			customize.mods[tostring(i)] = val

			customize.var[i] = v
			customize.var[tostring(i)] = v
		end
	end
...
RegisterNUICallback("pagar",function(data)
	if cart["total"] and func.checkPayment(cart["total"]) then
		...
		TriggerEvent("Progress","Aplicando Modificacoes",10000)
		Wait(10000)
		vRP.stopAnim(false)
		...
		-- pacote compatível com a garagem (entitydata / garages)
		local garageCustom = ToGarageCustomize(myveh)

		-- ✅ sempre pega por NATIVE (não depende do vRP.VehicleList)
		local vehplate = GetPlateFromVeh(vehicle)
		local vehname  = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)):lower()

		-- tenta pegar por VehicleList como complemento (algumas bases retornam o nome certo aqui)
		pcall(function()
			local _,_,p,n = vRP.VehicleList(7)
			if p and p ~= "" then vehplate = tostring(p):gsub("%s+","") end
			if n and n ~= "" then vehname = tostring(n):lower() end
		end)

		if vehplate and vehplate ~= "" then
			vehplate = tostring(vehplate):gsub("%s+","")

			-- ✅ Bennys tabela: salva no formato COMPAT (pra garagem aplicar sem nil)
			func.SaveModsByPlate(vehplate, garageCustom)
		end

		-- ✅ Garagem/entitydata: salva no formato que a garages aplica ao puxar
		if vehname and vehname ~= "" then
			func.saveVehicle(vehname, vehplate, garageCustom)
		end
	...
	if data then
		if data.type == "reparar" then
			...
			if func.repairVehicle(vehicle,damage) then
				...
				TriggerEvent("Progress","reparando",10000)
				Wait(10000)
				...
				myveh.damage = 0.0
				vRP.stopAnim(false)
				TriggerEvent("Notify","verde","Veículo reparado com <b>sucesso</b>.","Sucesso",7000)
...
function getAllVehicleMods(veh)
	...
	-- ✅ indica se a cor está em modo CUSTOM (RGB) ou só paleta
	myveh.isCustomPrimary = GetIsVehiclePrimaryColourCustom(veh)
	myveh.isCustomSecondary = GetIsVehicleSecondaryColourCustom(veh)
	...
	-- ✅ corrigido: usar o veh que veio por parâmetro
...
function setVehicleMods(veh,myveh,tunnerChip)
	...
	-- ✅ não mandar false
	local tint = myveh.windowtint
	if tint == false or tint == nil then tint = 0 end
...
function isVehicleTooFar(veh)
	Citizen.CreateThread(function()
		while nui do
			...
			if distance > 7 then
				fclient.closeNui()
				TriggerEvent("Notify","amarelo","Você se afastou muito do veículo.","Aviso",7000)
			end
			Citizen.Wait(500)
		end
	end)
