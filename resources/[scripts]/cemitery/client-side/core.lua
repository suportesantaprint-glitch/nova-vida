...
-----------------------------------------------------------------------------------------------------------------------------------------
local Init = {
	vec4(-1741.52,-219.85,56.14,255.12)
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- LOCATES
-----------------------------------------------------------------------------------------------------------------------------------------
...
CreateThread(function()
	for Index,v in pairs(Init) do
		exports.target:AddBoxZone("WorkCemitery:"..Index,v.xyz,0.75,0.75,{
			name = "WorkCemitery:"..Index,
			heading = v.w,
			minZ = v.z - 1.0,
			maxZ = v.z + 1.0
		},{
...
-- CEMITERY:INITBODY
-----------------------------------------------------------------------------------------------------------------------------------------
AddEventHandler("cemitery:initBody",function(Number)
	if Peds then
		if Selected then
	...
	if not Peds then
		...
		Selected = math.random(#Locates[Number])

		if LoadModel(List[Hash]) then
			Peds = CreatePed(26,List[Hash],Locates[Number][Selected].x,Locates[Number][Selected].y,Locates[Number][Selected].z - 1,Locates[Number][Selected].w - 180.0,false,false)
			...
			exports.target:AddCircleZone("Cemitery:"..Selected,vec3(Locates[Number][Selected].x,Locates[Number][Selected].y,Locates[Number][Selected].z - 0.75),0.5,{
