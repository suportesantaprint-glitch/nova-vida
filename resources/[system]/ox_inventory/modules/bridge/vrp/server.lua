...
-- Handle CharacterChosen event (vRP)
AddEventHandler("CharacterChosen", function(Passport, source, Creation)
    ...
    if Identity then
        player.name = Identity.Name .. " " .. Identity.Lastname
    end
...
-- Handle state bag updates for groups/jobs
SetTimeout(1000, function()
    local groups = vRP.Groups()
    if groups then
        for groupName in pairs(groups) do
            AddStateBagChangeHandler(groupName, nil, function(bagName, _, value)
                local source = GetPlayerFromStateBagName(bagName)
                if not source then return end
                local inventory = Inventory(source)
                if not inventory then return end
                inventory.player.groups[groupName] = value
            end)
        end
    end
end)
