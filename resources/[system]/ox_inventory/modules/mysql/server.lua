...
Citizen.CreateThreadNow(function()
    ...
    elseif shared.framework == 'qbx' then
        ...
        vehicleColumn = 'id'
    elseif shared.framework == 'vrp' then
...
function db.savePlayer(owner, inventory)
    return MySQL.prepare(Query.UPDATE_PLAYER, { inventory or '[]', owner })
end

function db.saveStash(owner, dbId, inventory)
    return MySQL.prepare(Query.UPSERT_STASH, { inventory or '[]', owner and tostring(owner) or '', dbId })
end
...
function db.saveGlovebox(id, inventory)
    return MySQL.prepare(Query.UPDATE_GLOVEBOX, { inventory or '[]', id })
end
...
function db.saveTrunk(id, inventory)
    return MySQL.prepare(Query.UPDATE_TRUNK, { inventory or '[]', id })
end
